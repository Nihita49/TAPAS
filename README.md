...
Wait for the live national heat-watch to finish loading.
Use the layer options to switch between Risk, Mortality, Hospitalization Spike, UTCI, and Vegetation.
Click through the map to drill down from India to a specific ward, and open the detail panel for that ward.
Check the Alert log for current alerts.
To test a scenario, turn on the Simulator and raise the temperature with the +°C control to preview escalation of measures and alerts. Select Live (reset) to return to real data.
Who It's For
